...
def teacher_model():
    ...
    x_gaf_0 = tf.keras.layers.Add()([x_g_0_0_t, x_g_1_0_t])
    x_skip_t = tf.keras.layers.Add()([x_f_1_t, x_gaf_0])
    ema_t = tf.train.ExponentialMovingAverage(decay=0.0001)
    ema_t.average(x_skip_t)
    
    x_g_0_1_t = SwinTransformer(
    dim=embed_dim,
    num_patch=(num_patch_x, num_patch_y),
    num_heads=8,
    window_size=4,
    ...
    x_gaf_1 = tf.keras.layers.Add()([x_g_0_1_t, x_g_1_1_t])
    x_t = tf.keras.layers.Add()([x_f_3_t, x_gaf_1])
